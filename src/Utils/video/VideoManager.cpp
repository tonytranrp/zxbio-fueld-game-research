...
void VideoManager::update() {
    ...
    for (auto& [_, inst] : m_videos) {
        ...
        if (result.completed) {
            Data::eventBus().trigger(event::video::VideoCompletedEvent{
                .videoName = inst.name
            });
            if (!inst.looping) {
                inst.playing = false;
...
void VideoManager::loadVideo(std::string_view name, std::string_view path) {
    ...
    if (!inst.backend || !inst.backend->load(inst.path, error)) {
        ...
        Data::eventBus().trigger(event::video::VideoErrorEvent{
...
void VideoManager::play(std::string_view name) {
    ...
    inst->paused = false;
    inst->ended = false;
    Data::eventBus().trigger(event::video::VideoStartedEvent{
        .videoName = inst->name
    });
}
