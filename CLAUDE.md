...
## Terrain pipeline (2026-09-08, Prompt 006 Group AM) -- operational deltas

Full record: `research/earth-terrain-pipeline-log.md`; how to run it: `docs/terrain-pipeline.md`.
Machine-relevant deltas ONLY:

- **The world is generated by a BAKED MACRO FIELD now, on by default.** `voxel_app` bakes an 8 km
  x 8 km field (500x500 at 16 m, 8.00 MB) in **0.545 s** before the first tree build; world-ready is
  **1.56 s**. `--no-macro-field` restores the old four-octave noise, which is the before column for
  every comparison. `--field-stages N` stops the pipeline after N stages.
- **The world origin MOVES per seed.** `recentre_on_land` shifts the field so the 512 m playable
  region is on land -- the log line says where ("world origin moved to (3648, 2512) in field
  space"). Without it the spawn is wherever the seed put it, and on the shipped seed that was open
  ocean. **A capture from one seed cannot be compared to another seed's by world coordinates.**
- **`tools/terrain_dump` is the terrain inspector**, and the first thing to reach for when the world
  looks wrong. `--plane elevation|flow|precip|temperature|biome|rivers`, `--zoom X,Z,SPAN` (world
  metres), `--seed`, `--stages`, `--cells`, `--cell-size`. It prints the ten acceptance tests, the
  per-biome stem densities, the river network, the climate statistics and the threshold sweeps.
  **Re-save its PNGs through PIL before committing** -- same stored-PNG lesson as `svo_render`.
- **`generate_column_heights_spaced` is the bulk path and it MUST agree with `height_at`.** It
  silently did not for the whole of this pass: it read the raw noise while `height_at` read
  macro + detail, so the rendered world and the collided world were different surfaces. If terrain
  ever looks unrelated to what the field dump shows, check that function first.
- **FastNoise2's `FractalFBm` is NOT normalised.** An N-octave stack at gain g spans +-sum(g^i), so a
  Remap from [-1,1] delivers that factor times the stated amplitude. `build_terrain_noise`'s "64 m"
  has always meant ~120 m. `build_detail_noise` divides by the weight sum; anything new that Remaps
  an FBm must too.
- **`DetailParams::amplitude_m` must be RE-SWEPT whenever the macro stages change.** It has moved
  four times (tables in its header) because it is not independent of the macro relief nor of the
  field extent. `ctest -R "detail parameters"` is the sweep and the gate.
- **Caves exist and the world is 3D below the surface** (goal 313, reopening goal 80).
  `CaveParams::threshold = 0` restores the exact 2.5D world and is what the
  `TerrainSampler`/`fill_terrain` byte-equivalence test runs with. **Any new code that concludes "this
  whole box is solid" must call `caves_possible_in_band` first** -- that is the only thing standing
  between caves and holes in the world.
- **The acceptance suite is a ctest gate over five seeds** and needs no GPU. Seven of ten pass on the
  shipped seed; the three that do not are goals 305/308/310's recorded results, each with a cause.
  **Do not tune them away.**
- **This world is a gentle coastal plain**: 8 km across, 112 m of relief, 0.50 C of temperature
  range, mean land slope 8 degrees. There is **no snowline** (freezing sits at 2,154 m, the highest
  ground is 44.6 m), **no cliff** (0 of 1,423 coast cells), rivers are **2.42 m wide against a 16 m
  cell**, and Whittaker's temperature axis does not exist. Several research anchors are quoted for
  landforms 10-20x larger, and the acceptance results have to be read against that.
- **398/398 tests.** New suites: `world/generation` (validation, biome, stencils, rivers, detail
  spectrum, five-seed acceptance) and `world/svo/tests/test_caves.cpp`.
