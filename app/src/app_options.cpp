...
constexpr std::array kTable{
    ...
    Option{.name = "aim-range",
           ...
           .group = "developer"},
    Option{.name = "skeleton-radius",
           .set = bind<&AppOptions::svo, &SvoWorldOptions::skeleton_radius>(),
           .kind = ValueKind::Float,
           .help = "metres within which trees voxelize from their grown skeleton (goal 336)",
           .default_text = "128 (the measured adopt lag; 0 = implicit shapes)",
           .group = "Rendering"},
    Option{.name = "grass-overlay",
           .set = bind<&AppOptions::svo_settings, &Settings::grass, &render::diligent::GrassOverlaySettings::enabled>(),
           .kind = ValueKind::Toggle,
           .help = "instanced raster grass blades in the near ring (goal 339)",
           .default_text = "off",
           .group = "Rendering"},
    Option{.name = "grass-overlay-radius",
           .set = bind<&AppOptions::grass_overlay_radius>(),
           .kind = ValueKind::Float,
           .help = "metres of raster grass overlay",
           .default_text = "14",
           .group = "Rendering"},
    Option{.name = "grass-overlay-max-blades",
           .set = bind<&AppOptions::grass_overlay_max_blades>(),
           .kind = ValueKind::Int,
           .help = "hard ceiling on drawn blades",
           .default_text = "131072 (the research's upper production figure)",
           .group = "Rendering"},
    Option{.name = "grass-overlay-density",
           .set = bind<&AppOptions::grass_overlay_density>(),
           .kind = ValueKind::Float,
           .help = "how much denser the raster ring is than the voxel tier",
           .default_text = "8",
           .group = "Rendering"},
    Option{.name = "grass-bend-radius",
           .set = bind<&AppOptions::svo_settings, &Settings::grass, &render::diligent::GrassOverlaySettings::player_bend_radius>(),
           .kind = ValueKind::Float,
           .help = "metres within which the body pushes blades aside (0 = off)",
           .default_text = "0.6",
           .group = "Rendering"},
    Option{.name = "grass-radius",
           .set = bind<&AppOptions::svo, &SvoWorldOptions::grass_radius>(),
           .kind = ValueKind::Float,
           .help = "metres of voxel ground cover in the finest ring (0 = none, goal 338)",
           .default_text = "24",
           .group = "Rendering"},
    Option{.name = "grass-tuft-plants",
           .set = bind<&AppOptions::svo, &SvoWorldOptions::grass_plants_per_tuft>(),
           .kind = ValueKind::Float,
           .help = "real plants one voxel tuft stands for; lower = denser (goal 338)",
           .default_text = "20",
           .group = "Rendering"},
    Option{.name = "sway",
           .set = bind<&AppOptions::sway>(),
           .kind = ValueKind::Toggle,
           .help = "hierarchical spring sway on the in-ring trees (goal 335)",
           .default_text = "on",
           .group = "Rendering"},
    Option{.name = "sway-radius",
           .set = bind<&AppOptions::sway_radius>(),
           .kind = ValueKind::Float,
           .help = "metres within which trees are swayed at all",
           .default_text = "120",
           .group = "Rendering"},
    Option{.name = "sway-max-trees",
           .set = bind<&AppOptions::sway_max_trees>(),
           .kind = ValueKind::Int,
           .help = "ceiling on swayed trees, taken nearest-first",
           .default_text = "512",
           .group = "Rendering"},
    ...
    Option{.name = "foveation",
           ...
           .default_text = "0",
           .group = "SVO shading"},
    // Prompt 007 goal 337 (= goal 192): C6.2's bounded experiment. Shipped state and the reason
    // for it are in goal 337's entry -- this is the knob its A/B captures were taken with.
    Option{.name = "anim-step",
           .set = bind<&AppOptions::svo_settings, &Settings::fixed_anim_step>(),
           .kind = ValueKind::Float,
           .help = "seconds the wind/wave clock advances per frame (0 = wall clock, goal 337)",
           .default_text = "0",
           .group = "developer"},
    Option{.name = "canopy-warp",
           .set = bind<&AppOptions::svo_settings, &Settings::canopy_warp_m>(),
           .kind = ValueKind::Float,
           .help = "metres of shading-domain warp on wind-responsive materials (0 = off)",
           .default_text = "0",
           .group = "SVO shading"},
    Option{.name = "grain",
