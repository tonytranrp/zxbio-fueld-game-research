...
// common/mod.rs is shared across four examples now (this one, voxel_scene, voxel_world,
// voxel_editing), each using a different subset -- this headless benchmark has no App/render
// loop, so it never calls common::log_fps_once_per_second. `#[allow(dead_code)]` at the
// inclusion site says that's
