# Fuel Farm Project Hub

This repo is now an Obsidian vault. Open this repository folder directly in Obsidian to browse the game as a linked knowledge graph.

## Start Here

- [[README|Repo overview]]
- [[Agents|Development master plan]]
- [[Notes/Obsidian Workflow|Obsidian workflow]]
- [[Notes/Source Map|Source map]]
- [[Notes/Research Map|Research map]]
- [[Bug/bug|Verified bug log]]

## Current Engineering Areas

- [[src/engine/README|Engine overview]]
- [[src/game/README|Game overview]]
- [[src/engine/physics/README|Rapier physics engine]]
- [[src/engine/world/README|World: terrain + voxel streaming]]
- [[src/game/screens/gameplay/README|Gameplay: walkable voxel world]]
- [[src/engine/graphics/README|Render and shader pipeline]]

## Project Notes

- [[Notes/Architecture Decisions|Architecture decisions]]
- [[Notes/Implementation Journal|Implementation journal]]
- [[Notes/Build and Verification|Build and verification]]

## Quick Tags

- #project/fuel-farm
- #area/engine
- #area/game
- #area/research
- #area/bugs
