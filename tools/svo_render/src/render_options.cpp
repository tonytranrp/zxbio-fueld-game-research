...
constexpr std::array kTable{
    ...
    Option{.name = "skeleton-radius",
           .set = bind<&Options::skeleton_radius>(),
           .kind = ValueKind::Float,
           .help = "metres within which trees voxelize from their grown skeleton (goal 336)",
           .default_text = "0 (implicit shapes)"},
    Option{.name = "macro-field",
           .set = bind<&Options::macro_field>(),
           .kind = ValueKind::Toggle,
           .help = "bake the macro terrain pipeline, as the app does",
           .default_text = "on"},
    Option{.name = "grass-radius",
           .set = bind<&Options::grass_radius>(),
           .kind = ValueKind::Float,
           .help = "metres of voxel ground cover in the finest ring (goal 338)",
           .default_text = "0"},
    Option{.name = "grass-tuft-plants",
           .set = bind<&Options::grass_plants_per_tuft>(),
           .kind = ValueKind::Float,
    ...
    Option{.name = "filter-albedo",
           .set = bind<&Options::filter_albedo>(),
           .kind = ValueKind::Toggle,
           .help = "band-limit the albedo toward the smoothing ancestor's material (goal 278)",
           .group = "Shading"},
