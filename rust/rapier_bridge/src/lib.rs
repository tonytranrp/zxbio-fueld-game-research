...
// Not re-exported by rapier3d::prelude -- the character-controller API lives
// in its own module and needs a direct import.
...
//
// The caller (C++ CharacterController3D) owns position authoritatively and
// passes it in explicitly rather than this function reading the collider's
// own cached world pose: PhysicsSystem::stepFixed() runs before
// ScreenManager's screen update each frame, so a setBodyPosition() issued
// from a screen wouldn't be visible in the collider's cached pose until the
// *next* step -- reading col.position() here would use last frame's
// position. Passing it explicitly removes that one-step staleness hazard.
//
// Zero unsafe/unwrap/expect/panic!, matching every other function in this
// crate (panic = "abort" is set workspace-wide, so a panic here would kill
// the whole host process, not just this call).
